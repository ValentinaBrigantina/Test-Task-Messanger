...
export function MainChannel() {
  ...
  const targetContactQuery =
    context?.currentTargetChannel &&
    useQuery(
      getTargetContactByChannelIdQueryOptions(context?.currentTargetChannel)
    )
    ...
    <div className="basic-3/4 flex-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-lg leading-9 tracking-tight text-ring">
          {context && context?.currentTargetChannel?.isGroup
            ? `Channel ${context?.currentTargetChannel?.name}`
      ...
      <div className="container px-0 overflow-auto h-4/5 flex flex-col-reverse shadow-xl rounded-md">
        <div className=" bg-neutral-700 flex flex-col justify-end">
          {context?.currentTargetChannel && (
            <MessagesChannel channel={context?.currentTargetChannel} />
          )}
          {context?.currentTargetChannel && (
            <SendMessageForm channel={context?.currentTargetChannel} />
          )}
        </div>
      </div>
    </div>
